import { Link } from 'react-router-dom';
import { ArrowLeft, Download } from 'lucide-react';
import { useQuery } from '@tanstack/react-query';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { PageHeader } from '@/components/shared/PageHeader';
import { EmptyState } from '@/components/shared/EmptyState';
import { ReportFilterBar, useReportFilters } from '@/components/reports/ReportFilters';
import { reportTaskVelocity } from '@/lib/reports';
import { toCsv, downloadCsv } from '@/lib/csv-export';

export function ReportTaskVelocityPage() {
  const { branch, from, to } = useReportFilters();

  const { data, isLoading } = useQuery({
    queryKey: ['reports', 'task-velocity', branch, from, to],
    queryFn: () => reportTaskVelocity({ branch, from, to }),
  });

  const rows = data ?? [];
  const maxVal = Math.max(1, ...rows.map((r) => Math.max(r.created, r.finished)));

  const handleExport = () => {
    const csv = toCsv(
      ['week_start', 'created', 'finished'],
      rows as unknown as Record<string, unknown>[],
    );
    downloadCsv('task-velocity.csv', csv);
  };

  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <Link to="/reports" className="text-muted-foreground hover:text-foreground inline-flex items-center text-sm">
        <ArrowLeft className="mr-1 h-4 w-4" /> Back to reports
      </Link>

      <PageHeader
        eyebrow="Report"
        title="Task velocity"
        actions={
          <Button size="sm" variant="outline" onClick={handleExport} disabled={rows.length === 0} data-testid="csv-export">
            <Download className="mr-1 h-4 w-4" /> Export CSV
          </Button>
        }
      />

      <ReportFilterBar />

      {isLoading && <div className="text-muted-foreground p-6 text-sm">Loading report…</div>}

      {!isLoading && rows.length === 0 && (
        <Card><CardContent className="p-0">
          <EmptyState title="No data for this period" tone="muted" size="compact" />
        </CardContent></Card>
      )}

      {!isLoading && rows.length > 0 && (
        <Card>
          <CardContent className="p-5">
            <div className="overflow-x-auto">
              <table className="w-full text-sm" data-testid="report-table">
                <thead>
                  <tr className="border-border border-b text-left">
                    <th className="text-foreground-56 pb-2 pr-4 text-xs font-medium">Week</th>
                    <th className="text-foreground-56 pb-2 pr-4 text-xs font-medium">Created</th>
                    <th className="text-foreground-56 pb-2 pr-4 text-xs font-medium">Finished</th>
                    <th className="text-foreground-56 pb-2 text-xs font-medium">Chart</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.week_start} className="border-border border-b last:border-0">
                      <td className="py-2 pr-4 tabular-nums">{r.week_start}</td>
                      <td className="py-2 pr-4 tabular-nums">{r.created}</td>
                      <td className="py-2 pr-4 tabular-nums">{r.finished}</td>
                      <td className="py-2">
                        <svg width="120" height="16" className="inline-block">
                          <rect x={0} y={0} height={7} width={Math.max(1, (r.created / maxVal) * 120)} className="fill-primary/60" rx={2} />
                          <rect x={0} y={9} height={7} width={Math.max(1, (r.finished / maxVal) * 120)} className="fill-success/60" rx={2} />
                        </svg>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          </CardContent>
        </Card>
      )}
    </div>
  );
}
